###############################################################################
#
# FILE: reallocation_shift_switch_best.R
#
# BY: Dmitry Sedov 
#
# DATE: Fri Sep 4 2020
...
#       using both shifting firms and switching their locations.
#
###############################################################################
...
# Get the mapping between restaurants and locations
...
by_cbg_initial <- rest_location_initial %>% 
  ...
  summarise(total_pop = first(total_pop), 
            ...
            markup = mean(markup, na.rm = T)
  )

# Compute market characteristics: correlations
...
count_density <- with(by_cbg_initial, cor(restaurant_count, total_pop))

markup_density <- with(by_cbg_initial, cor(markup, total_pop, use = 'complete.obs'))
markup_count <- with(by_cbg_initial, cor(markup, restaurant_count, use = 'complete.obs'))
markup_distance <- with(rest_location_initial, cor(markup, weighted_distance, use = 'complete.obs'))
...
output_data <- data.frame(cbsa = cbsa,
                          ...
                          markup_density = markup_density, 
                          markup_count = markup_count, 
                          markup_distance = markup_distance, 
                          n_shifts = 0, 
                          n_switches = 0)

# Rho-equivalent utilities
x <- seq(-0.5, -0.01, 0.01)
y <- sapply(x, 
...
stats <- shift_switch_improve_allocation(max_time)
...
count_density <- with(by_cbg, cor(restaurant_count, total_pop))

markup_density <- with(by_cbg, cor(markup, total_pop, use = 'complete.obs'))
markup_count <- with(by_cbg, cor(markup, restaurant_count, use = 'complete.obs'))
markup_distance <- with(rest_location_final, cor(markup, weighted_distance, use = 'complete.obs'))
