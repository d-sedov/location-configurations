...
################################################################################
#
# FILE: commercialexchange-retail-rent-download.py
#
# BY: Dmitry Sedov 
#
# CREATED: Mon Mar 30 2020
#
# DESC: This code downloads rent data from commercialexchange.com.
#
# EXEC: python3 commercialexchange-retail-rent-download.py search
#       python3 commercialexchange-retail-rent-download.py results
#      
################################################################################
################################################################################
...
url_search = 'https://api.commercialexchange.com/api/cfra/ee/search/properties/_list?pageSize=50&pageNumber=1&sort=QUALITY%3Adesc&byProperty=true'
...
output_folder_path_results = '/home/user/projects/urban/data/input/Rent/commercialexchange/retail/results'
...
form_data = {'listedStatus':['AVAILABLE_LEASE'],
             'propertyCategories':{'RETAIL':[]},
             'spaceCategories':['RETAIL']
            }
    ...
    async with sem:
        while attempt != 0:
            try:
                ...
                with open(os.path.join(code_folder_path, 'commercialexchange-search.log'), 'a+') as log_file:
    ...
    async with sem:
        while attempt != 0:
            try:
                ...
                with open(os.path.join(code_folder_path, 'commercialexchange-results.log'), 'a+') as log_file:
...
if __name__ == '__main__':
    ...
    if part == 'search':
        ...
        search_results.to_csv(os.path.join(output_folder_path_search, 'commercialexchange-search.csv'), index = False)
        print('Done.')
