...
create_yelp_copy_statement = """
CREATE TEMPORARY TABLE yelp_copy AS (
    SELECT 
        *,
...
CREATE INDEX yelp_copy_location_geography_idx
ON yelp_copy
...
CREATE TEMPORARY TABLE match_candidates AS (
    ...
    LEFT JOIN
        yelp_copy AS y
    ON 
        ST_DWithin(r.location, y.location_geography, 30)
    ORDER BY 
        r.sname_place_id,
        distance
);
...
def match_candidates_by_distance(con):
    ...
    # Create a copy of yelp_restaurants table
    logger.info('Creating Yelp copy.')
    l_create_yelp_copy_statement = (
            create_yelp_copy_statement
            )
    con.execute(l_create_yelp_copy_statement)
